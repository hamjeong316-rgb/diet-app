...
# ── CSS ──────────────────────────────────────────────────
from styles import MAIN_CSS, LOGIN_TAB_CSS, LOGIN_HERO_HTML
...
if st.session_state.uid is None:
    st.markdown(LOGIN_TAB_CSS,   unsafe_allow_html=True)
    st.markdown(LOGIN_HERO_HTML, unsafe_allow_html=True)

    tab_sel, tab_new = st.tabs([":material/person: 사용자 선택", ":material/person_add: 신규 등록"])
    ...
    with tab_new:
        ...
        with st.form("new_user_form", clear_on_submit=False):
            ...
            new_act    = st.selectbox("활동량",
                         ["거의 없음","약간","보통","활발함","매우 활발"], index=2)
        ...
        if reg_btn:
            if not new_name.strip():
                ...
                new_data = {
                    "id": None,
                    "name": new_name.strip(), "age": new_age, "gender": new_gender,
                    "height": new_height, "weight": new_weight,
                    "body_fat_pct": new_bfat, "muscle_mass": new_muscle,
                    "bmr": round(new_bmr, 1), "tdee": round(new_tdee, 1),
                    "visceral_fat": new_visc, "body_fat_mass": new_fmass,
                    "goal_weight": new_gwt, "goal_days": new_gday,
                    "goal_start_date": new_gstart.isoformat(),
